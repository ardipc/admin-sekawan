...
export default function CreateEvent(){
    ...
    const fetchData = async () => {
        setLoad(true);
        const {data, error} = await supabase.from('metadata').select('id, create_format_proposal, create_tnc').eq('id', 1).single();
    ...
    const upload = async () => {
        ...
        if(proposal) {
            // delete existing
            if(meta.create_format_proposal) {
                await deleteFileOnStorage(supabase, meta.create_format_proposal);
            }

            // upload proposal
            const fullpath = await uploadFileToStorage(supabase, proposal[0]);
            await supabase.from("metadata").update({ create_format_proposal: fullpath }).eq('id', 1);
        }

        if(tnc) {
            if(meta.create_tnc) {
                await deleteFileOnStorage(supabase, meta.create_tnc);
            }

            // upload proposal
            const fullpath = await uploadFileToStorage(supabase, tnc[0]);
            await supabase.from("metadata").update({ create_tnc: fullpath }).eq('id', 1);
        }
                    ...
                    <div className="flex gap-4 items-start">
                        <div className="form-control w-full max-w-xl mt-4">
                            ...
                            <input key={keyProposal} onChange={e => e.target.files && setProposal(e.target.files)} type="file" className="file-input file-input-bordered file-input-primary w-full max-w-xl rounded-none" />
                            {
                                ...
                                <>
                                    {
                                        meta?.create_format_proposal &&
                                        <div className="border mt-3 p-4">
                                            <Link target="_blank" href={meta.create_format_proposal} className="btn btn-primary w-full">View File</Link>
                                        </div>
                                    }
                                </>
                            }
                                ...
                                <>
                                    {
                                        meta?.create_tnc &&
                                        <div className="border mt-3 p-4">
                                            <Link target="_blank" href={meta.create_tnc} className="btn btn-primary w-full">View File</Link>
                                        </div>
                                    }
                                </>
                            }
