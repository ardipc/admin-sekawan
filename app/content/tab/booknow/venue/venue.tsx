...
export default function Venue({ setActive, setItem }: { setActive: any; setItem: any; }){
    ...
    const fetchData = async () => {
        setIsLoad(true);
        const {data, error} = await supabase.from("facilities").select().eq('content', 0).eq('tipe', 'venue').order('id', { ascending: false });
        ...
        <>
            ...
            <div className="flex justify-between">
                <div className="flex gap-2 items-center">
                    ...
                    <p className="text-3xl font-bold">List Venue</p>
                </div>
                <button onClick={()=> {setItem(null); setActive(1); }} className="btn btn-primary rounded-none text-white text-xs font-bold"><img src="/icons/tambah-wt.svg" alt="tambah" />Tambah Venue</button>
            </div>
                                ...
                                <tr key={`sdf-${index}`}>
                                    ...
                                    <td>
                                        <div className="flex gap-2">
                                            <button onClick={() => { setItem(item); setActive(1);}} type="button" className="btn btn-sm btn-primary rounded-none">
