...
export default function PVFacilities(){
    ...
    const fetchLists = async () => {
        ...
        const { data } = await supabase.from("bookings")
            .select("*, banks(name, logo, nomor_rekening, account_owner), bookings_provements(id, filename, created_at), facilities_packages(*, facilities(*)), bookings_user(name, phone, email)")
            .eq('content', 0)
            .eq('tipe', 'facility')
    ...
    const rejectPayment = async () => {
        await supabase.from("bookings").update({ status: "Purchase Canceled By Admin", feedback }).eq('id', booking.id);
        await supabase.from("bookings_provements").delete().eq('booking_id', booking.id);
        fetchLists();
        closeModalProovement();
    }
        ...
        <>
            <div className="content p-6">
                <div className="flex gap-2 items-center">
                    ...
                    <p className="text-3xl font-bold">List Booking Facilities</p>
                </div>
                    ...
                    <table className="table">
                        ...
                        <thead className="py-4 px-6 bg-primary/10 border-primary border-2">
                            <tr>
                                ...
                                <th className="text-primary font-bold text-sm">Tanggal</th>
                                ...
                                isLoad ?
                                    ...
                                    lists.map((item: any, index: any) => (
                                        <tr key={`sdf-${index}`}>
                                            ...
                                            <td>{moment(item.start_date).format('DD MMM YYYY (HH:mm)')} <br/> {moment(item.end_date).format('DD MMM YYYY (HH:mm)')}</td>
                                            <td>{item.facilities_packages.name}</td>
                                            <td>{item.facilities_packages.facilities.name}</td>
                                            <td>
                            ...
                            <>
                                <div className="mt-6">
                                    <div className="booking-details1 border-2 p-4 mb-3">
                                        <h1 className="text-2xl font-bold text-black">Facility</h1>
                                        <div className="columns columns-2 gap-4 mt-4">
                                            <img src={booking.facilities_packages.facilities.cover} alt="kursi" />
                                            <p className="text-xl font-bold text-black">{booking.facilities_packages.facilities.name}</p>
                                            <h1 className="text-lg mt-2 text-black">{booking.facilities_packages.name}</h1>
                                        </div>
                                    </div>
                                    ...
                                    <div className="booking-details2 border-2 p-4 mb-3">
                                        ...
                                        <div className="gap-4 mt-4">
                                            <div className="flex mb-4 items-center">
                                                ...
                                                <p className="text-xl font-bold text-black ms-3">{booking.banks.name}</p>
                                            </div>
...

                                            {
                                                booking.bookings_provements.map((item: any, index: number) => item.filename && (
                                                    <img key={`gmb-${index}`} src={item.filename} alt="Gambar" className="w-full my-1" />
                                                ))
                                            }
